...
const feature = {
  type: 'Feature',
  properties: {},
  geometry: { type: 'LineString', coordinates: [[127.12, 37.44], [127.14, 37.48]] }
};
...
describe('routing client', () => {
  ...
  it.each([
    ...
    ['숫자', { profileid: 123 }],
    ['빈 문자열', { profileid: '' }]
  ])('업로드된 profileid가 %s이면 서비스 오류를 반환하고 캐시하지 않는다', async (_, profileData) => {
    const fetchImpl = vi.fn((input, options) => {
      if (options?.method === 'POST') {
        return Promise.resolve(ok(profileData));
      }
      return Promise.resolve(routeResponse());
    });
    const client = createRoutingClient({ fetchImpl });
    ...
    expect(profileUploadCount(fetchImpl)).toBe(3);
    expect(routeRequestCount(fetchImpl)).toBe(0);
  });
  ...
  it.each([
    ...
    ['geometry type이 LineString이 아닌 응답', ok({ features: [{ type: 'Feature', geometry: { type: 'Point', coordinates: [[127.12, 37.44], [127.14, 37.48]] } }] })],
    ['geometry coordinates가 배열이 아닌 응답', ok({ features: [{ type: 'Feature', geometry: { type: 'LineString', coordinates: 'not an array' } }] })],
    ['좌표가 두 개 미만인 응답', ok({ features: [{ type: 'Feature', geometry: { type: 'LineString', coordinates: [[127.12, 37.44]] } }] })],
    ['좌표 경도가 범위를 벗어난 응답', ok({ features: [{ type: 'Feature', geometry: { type: 'LineString', coordinates: [[181, 37.44], [127.14, 37.48]] } }] })],
    ['좌표가 유한한 숫자가 아닌 응답', ok({ features: [{ type: 'Feature', geometry: { type: 'LineString', coordinates: [[127.12, 'not a number'], [127.14, 37.48]] } }] })]
  ...
  it('같은 만료 프로필의 동시 실패에는 모드별 재등록을 한 번만 수행한다', async () => {
    ...
    const fetchImpl = vi.fn((input, options) => {
      ...
      const url = String(input);
      if (url.includes('profile=profile_0')) {
        return Promise.resolve({ ok: false });
      }
      return Promise.resolve(routeResponse());
    });
    ...
    expect(profileUploadCount(fetchImpl)).toBe(4);
    expect(routeRequestCount(fetchImpl)).toBe(8);
  });
});
